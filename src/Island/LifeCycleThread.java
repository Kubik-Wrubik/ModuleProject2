		...
		for(int i = 0; i < island.getLength(); i++){
			for(int j = 0; j < island.getWidth(); j++){
				...
				for(Meat meat : island.cells[i][j].getMeat()){
					builder.append(String.format("%s(%.2f)", PseudoGraphic.iconTransform(meat), meat.weight));
				}

				Vegetation vegetation = island.cells[i][j].getVegetation();
				builder.append(String.format("%s(%.2f)", PseudoGraphic.iconTransform(vegetation), vegetation.currentPlants));
